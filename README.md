...
The optimization requires around 50 iterations and only a few seconds on a modern workstation. The resulting capacitance values and B<sub>1</sub><sup>+</sup> field magnitude plot are provided in *Table 2* and in the following figure, respectively.

*Table 2*
| Capacitor number | Optimized Capacitance [pF] |
|---|---|
|C1	| 23.8|
|C2	| 21.6|
|C3	| 16.8|
...
This example illustrates the power of co-simulation in RF coil design. The optimal capacitors are determined only from one full-wave simulation and a few seconds of post-processing on a modest PC, using a simple Python script and the open-source [CoSimPy library](https://github.com/umbertozanovello/CoSimPy).
